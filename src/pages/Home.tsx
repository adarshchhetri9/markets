          ...
          <Link className="flex " to={`/${coin.id}`}>
            <div className="flex gap-2 items-center">
              ...
              <div>
                ...
                <div className="w-[100px] text-slate-400 font-thin">{coin.name}</div>
              </div>
            </div>
            {coin.priceBtc && (
              <div className=" ">
                <ResponsiveContainer width={120} height={40}>
                  <LineChart data={store.graphData[coin.id]} margin={{ top: 5, right: 30, left: 10, bottom: 5 }}>
                    <Line type="monotone" dataKey="Price" stroke="#81f542" dot={false} />
                  </LineChart>
                </ResponsiveContainer>
              </div>
            )}
            {coin.priceBtc && (
              <div>
                <div className="flex justify-start gap-1 items-center">
                  ...
                  <h1 className="relative overflow-hidden text-xs font-semibold  ">{coin.priceBtc}</h1>
                </div>
                <div className="ml-[2.5px] flex justify-start gap-2 items-center mt-2 ">
                  <img src={money} className="w-[15px] h-{15px]" alt="" />
                  <h1 className="relative  overflow-hidden text-[10px] text-slate-800  ">{coin.priceUsd}</h1>
                  <h1></h1>
                </div>
              </div>
            )}
